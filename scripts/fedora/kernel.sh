...
retry dnf --assumeyes install kernel-tools kernel-devel kernel-headers; error
...
if [[ $(rpm -q kernel | wc -l) != 1 ]]; then
  dnf --setopt=protected_packages= --assumeyes remove "$(dnf repoquery --installonly --latest-limit=-1 -q)"
  error
fi
